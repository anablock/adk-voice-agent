...
async def root():
    ...
    return {"status": "online", "service": "ADK Voice Agent API"}

@app.get("/api/status", dependencies=[Depends(verify_api_key)])
async def status():
    """API status endpoint that returns active sessions count"""
    return {
        "status": "online", 
        "active_sessions": len(active_sessions),
        "service": "ADK Voice Agent API"
    }
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main_gcp:app", host="0.0.0.0", port=port)
