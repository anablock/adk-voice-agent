        ...
        async for event in live_events:
            ...
            try:
                if part.text is not None:
                    ...
                    # Handle executable code (usually Python code from the agent)
                    print(f"[AGENT TO CLIENT]: Executable code: {part.executable_code}")
                    # We don't send this to the client directly, it's executed by the agent internally
                    pass
                else:
                    # Try to convert the part to a string representation
                    try:
                        part_str = str(part)
                        print(f"[AGENT TO CLIENT]: Other part type, sending as text: {part_str[:50]}...")
                        message = {
                            "mime_type": "text/plain",
                            "type": "text/plain",
                            "data": f"Agent is processing: {part_str}",
                            "content": f"Agent is processing: {part_str}",
                            "role": "model",
                            "partial": event.partial,
                        }
                        await websocket.send_text(json.dumps(message))
                    except:
                        print("[AGENT DEBUG]: Unsupported part type and couldn't convert to string")
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main_final_fix:app", host="0.0.0.0", port=port, reload=False)
