#!/bin/bash
# Start script for Fixed ADK Voice Agent

echo "=== Fixed ADK Voice Agent Server Start ==="
echo "Stopping any existing voice agent servers..."
pkill -f "python -m app.fixed_adk_server" || true
...
echo "Starting fixed voice agent server..."
cd "$(dirname "$0")"
export PYTHONPATH="$(pwd):$PYTHONPATH"
python -m app.fixed_adk_server > fixed_server.log 2>&1 &
server_pid=$!
...
if ps -p $server_pid > /dev/null; then
    ...
    echo "Log file: $(pwd)/fixed_server.log"
    # Verify it's listening on the right port
    if lsof -i:8081 -a -p $server_pid > /dev/null; then
        echo "Server is listening on port 8081"
    else
        echo "Warning: Server may not be listening on port 8081 yet"
        tail -5 fixed_server.log
    fi
else
    echo "Error: Server failed to start. Check fixed_server.log for details:"
    tail -20 fixed_server.log
    exit 1
fi
